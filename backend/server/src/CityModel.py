...
class CityModel(ap.Model):
    @staticmethod
    def __get_positions(grid: list) -> list:
        positions = []
    ...
    def setup(self):
        ...
        self.spawn_points: list = self.p["spawn_points"]
        self.size: tuple = self.p["size"]
    ...
    def get_frame(self) -> str:
        ...
        for agent in self.grid.agents:
            if agent.type == "CarAgent":
                ...
                data['trafficLights'].append({
                    'id': agent.id,
                    'pos': self.grid.positions[agent],
                    'color': agent.get_current_light().value,
